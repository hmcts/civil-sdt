...
DECLARE i_purge_job_id NUMERIC;
  ...
  c1 CURSOR (n_param DOUBLE PRECISION)
  ...
  c2 CURSOR (n_param DOUBLE PRECISION)
  ...
  c3 CURSOR (n_param DOUBLE PRECISION)
  ...
  --   with service requests created before given retention date
  c4 CURSOR (n_param DOUBLE PRECISION)
  ...
  BEGIN
    ...
    VALUES (i_purge_job_id, now(), 'Purge retention period is ' || i_retention_period ||
    ...
    n_iteration := 0;
    FOR c4_rec IN c4(i_retention_period) LOOP
      ...
      IF MOD(n_iteration,i_CommitInterval)=0 THEN
        /* COMMIT; */
      END IF;
    END LOOP;
    ...
    INSERT INTO sdt_owner.purge_job_audit_messages
    (purge_job_id, created_date,log_message)
    VALUES (i_purge_job_id, now(), n_iteration || ' records deleted from error_logs table.');
    ...
    n_iteration := 0;
    FOR c3_rec IN c3(i_retention_period) LOOP
    ...
    INSERT INTO sdt_owner.purge_job_audit_messages
    (purge_job_id, created_date,log_message)
    VALUES (i_purge_job_id, now(), n_iteration || ' records deleted from individual_requests table.');
    -- update count of deletions - error logs
    UPDATE sdt_owner.purge_job_audit
    ...
    n_iteration := 0;
    FOR c2_rec IN c2(i_retention_period) LOOP
    ...
    INSERT INTO sdt_owner.purge_job_audit_messages
    (purge_job_id, created_date,log_message)
    VALUES (i_purge_job_id, now(), n_iteration || ' records deleted from bulk_submissions table.');
    UPDATE sdt_owner.purge_job_audit
    ...
    n_iteration := 0;
    FOR c1_rec IN c1(i_retention_period) LOOP
    ...
    INSERT INTO sdt_owner.purge_job_audit_messages
    (purge_job_id, created_date,log_message)
    VALUES (i_purge_job_id, now(), n_iteration || ' records deleted from service_requests table.');
    UPDATE sdt_owner.purge_job_audit
  	SET count_of_service_requests=n_iteration
  	WHERE purge_job_id=i_purge_job_id;
