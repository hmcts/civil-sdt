...
-----------------------------------------
\copy (SELECT (SELECT 'BULK_CUST_APP_SEQ' || ',' || last_value FROM bulk_cust_app_seq), (SELECT 'BULK_CUST_SEQ' || ',' || last_value FROM bulk_cust_seq), (SELECT 'BULK_SUB_SEQ' || ',' || last_value FROM bulk_sub_seq), (SELECT 'ERR_LOG_SEQ' || ',' || last_value FROM err_log_seq), (SELECT 'ERR_MESG_SEQ' || ',' || last_value FROM err_mesg_seq), (SELECT 'GLB_PAR_SEQ' || ',' || last_value FROM glb_par_seq), (SELECT 'IND_REQ_SEQ' || ',' || last_value FROM ind_req_seq), (SELECT 'SDT_REF_SEQ' || ',' || last_value FROM sdt_ref_seq), (SELECT 'SER_ROU_SEQ' || ',' || last_value FROM ser_rou_seq), (SELECT 'SER_TYP_SEQ' || ',' || last_value FROM ser_typ_seq), (SELECT 'SRV_REQ_SEQ' || ',' || last_value FROM srv_req_seq), (SELECT 'TAR_APP_SEQ' || ',' || last_value FROM tar_app_seq)) to 'validation_sequences.txt';
